...
static bool ReleaseAudioClip(struct AudioClip *clip) {
  SDL_free(clip->wavBuffer);
  SDL_free(clip);
  return true;
}

static bool ReleaseAllClips(struct ClipNode *clipNode) {
  while (clipNode) {
    struct ClipNode *currentNode = clipNode;
    clipNode = currentNode->next;
    if (!ReleaseAudioClip(currentNode->clip)) {
      return false;
    }
...
static bool ReleaseAudioBus(struct AudioBus *bus) {
  if (!SDL_ClearAudioStream(bus->stream)) {
    SDL_Log("Could not clear audio stream, %s", SDL_GetError());
    return false;
  }
  SDL_DestroyAudioStream(bus->stream);
  if (!ReleaseAllClips(bus->headClip)) {
    return false;
  }
  SDL_free(bus);
  return true;
...
static bool ReleaseAllBuses() {
  while (headBus) {
    struct BusNode *currentNode = headBus;
    headBus = currentNode->next;
    if (!ReleaseAudioBus(currentNode->bus)) {
      return false;
    }
    SDL_free(currentNode);
  }
...
static void SDLCALL PostmixCallback(void *userdata, const SDL_AudioSpec *spec,
...
void AudioPlayer_Init() {
  ...
  if (!SDL_SetAudioPostmixCallback(audioDeviceId, PostmixCallback, NULL)) {
...
static void StreamAudioClip(struct AudioClip *clip, Sint16 *busSamples,
  ...
  int bufferSize =
      SDL_min(additionalAmount, clip->wavLength - clip->audioLength);
...
static void SDLCALL BusCallback(void *bus, SDL_AudioStream *stream,
  ...
  SDL_memcpy(copiedSamples, ((const Sint16 *)samples),
             (size_t)additionalAmount);

  Uint8 mixBuffer[additionalAmount];
  SDL_memset(mixBuffer, '\0', additionalAmount);
  if (!SDL_MixAudio(mixBuffer, copiedSamples, SDL_AUDIO_S16, additionalAmount,
                    SDL_min(currentBus->currentVolume, 128))) {
...
struct AudioBus *AudioPlayer_NewBus() {
  struct AudioBus *newAudioBus =
      (struct AudioBus *)SDL_malloc(sizeof(struct AudioBus));
  SDL_AudioStream *stream = SDL_OpenAudioDeviceStream(audioDeviceId, &audioSpec,
                                                      BusCallback, newAudioBus);
  if (!stream) {
  ...
  struct BusNode *newBusNode =
      (struct BusNode *)SDL_malloc(sizeof(struct BusNode));
...
struct AudioClip *AudioPlayer_NewAudioClip(struct AudioBus *bus,
                                           const char *path) {
  struct AudioClip *newAudioClip =
      (struct AudioClip *)SDL_malloc(sizeof(struct AudioClip));
  if (!SDL_LoadWAV(path, &newAudioClip->spec, &newAudioClip->wavBuffer,
                   &newAudioClip->wavLength)) {
  ...
  newAudioClip->msDuration =
      ((double)sampleLen / (double)newAudioClip->spec.freq) * 1000;
  ...
  newAudioClip->loop = false;
  struct ClipNode *newClipNode =
      (struct ClipNode *)SDL_malloc(sizeof(struct ClipNode));
...
static void FadeOutClip(struct AudioClip *clip, float duration,
...
void AudioPlayer_AudioClipSetLoop(struct AudioClip *clip, bool loop) {
  if (!clip)
    return;
  clip->loop = loop;
}
